...
#include "perifericos/plic_y_pcr.h"
#include "perifericos/plic_y_pcr.cpp"
...
public:
        ...
        int SimulateCore(const std::string &progfile, unsigned long max_time=1000000L) {
                ...
                for (; getTime() < max_time;) {
                    for(int i = 1; i < 32 ; i++){
                        printf("Prueba de PLIC:\n");
                        Tick();
                        tests.sim(m_core->wbm_io_addr_o, m_core->wbm_io_dat_o, m_core->wbm_io_sel_o, m_core->wbm_io_cyc_o, m_core->wbm_io_stb_o,
                                   m_core->wbm_io_we_o, m_core->wbm_io_dat_i, m_core->wbm_io_ack_i, m_core->wbm_io_err_i, m_core->xinterrupts_i, 
                                   m_core->Algol__DOT__xint_meip, m_core->Algol__DOT__xint_msip,m_core->Algol__DOT__xint_mtip);
                        m_core->xinterrupts_i = m_core->xinterrupts_i<<1;
                    }
                    Tick();
